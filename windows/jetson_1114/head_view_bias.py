...
def _sector_from_delta(dx: float, dy: float) -> str:
    ...
    if  -22.5 < ang <=  22.5:  return "E"
    if   22.5 < ang <=  67.5:  return "NE"
    if   67.5 < ang <= 112.5:  return "N"
    if  112.5 < ang <= 157.5:  return "NW"
    if   ang > 157.5 or ang <= -157.5: return "W"
    if -157.5 < ang <= -112.5: return "SW"
    if -112.5 < ang <=  -67.5: return "S"
    # -67.5 < ang <= -22.5
    return "SE"
...
class HeadViewBias3DParams:
    ...
    min_dist_m: float = 0.01   # 이하면 보정 0
    max_dist_m: float = 0.80   # 이상이면 보정 100%. min_dist_m ~ max_dist_m 사이에 있으면 거리에 비례해서 가중치 up
    deadband_m: float = 0.10   # 얼마만큼 있으면 보정을 안할거냐
    
    # 타깃 평면에서 허용할 최대 보정 오프셋(미터). 벽에서 최대 몇 cm 까지 이동시킬건지.
    max_offset_m: float = 0.07  # 7 cm
    
    # 각도 상한(절대, 도) — “1도 이하” 요구 반영(거리 짧아도 0.9° 캡). 거리가 가까운 상황에서 제한하기.
    max_angle_cap_deg: float = 1.2
    
    gamma: float = 0.6 # 낮게 하면 가까이 있어도 가중치가 빨리 증가한다.(0 ~ 1 범위)
