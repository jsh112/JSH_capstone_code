...
def interactive_select_both(Limg, Rimg, holdsL, holdsR, swap_display=False, window="Select Holds (L|R)"):
    W = Limg.shape[1]
    H = Limg.shape[0]
    selector = BiClickSelector(holdsL, holdsR, W, swap_display)
    cv2.namedWindow(window, cv2.WINDOW_NORMAL)
    cv2.setMouseCallback(window, selector.on_mouse)

    while True:
        vis = (cv2.hconcat([Rimg, Limg]) if swap_display else cv2.hconcat([Limg, Rimg])).copy()

        # --- 모든 컨투어: 검정(두께 2)로 얇게 ---
        for side, holds in (("L", holdsL), ("R", holdsR)):
            xoff = (0 if (side == ("L" if not swap_display else "R")) else W)  # 좌측 뷰의 x오프셋 0/W
            shift = np.array([[[xoff, 0]]], dtype=np.int32)
            for h in holds:
                cnt_shifted = h["_cnt"] + shift
                cv2.drawContours(vis, [cnt_shifted], -1, (0, 0, 0), 2, cv2.LINE_AA)

        # --- 선택된 것: 노란색(두께 3) + L/R 번호 ---
        for n, i in enumerate(selector.selL, start=1):
            h = holdsL[i]
            shift = np.array([[[0 if not swap_display else W, 0]]], dtype=np.int32)
            cnt = h["_cnt"] + shift
            cv2.drawContours(vis, [cnt], -1, (0, 255, 255), 3, cv2.LINE_AA)
            cx, cy = h["center"]
            cx += (0 if not swap_display else W)
        ...
        for n, j in enumerate(selector.selR, start=1):
            h = holdsR[j]
            shift = np.array([[[W if not swap_display else 0, 0]]], dtype=np.int32)
            cnt = h["_cnt"] + shift
            cv2.drawContours(vis, [cnt], -1, (0, 255, 255), 3, cv2.LINE_AA)
            cx, cy = h["center"]
            cx += (W if not swap_display else 0)
            cv2.putText(vis, f"R{n}", (cx + 8, cy - 8),
                        cv2.FONT_HERSHEY_SIMPLEX, .6, (0, 0, 0), 2, cv2.LINE_AA)
            cv2.putText(vis, f"R{n}", (cx + 8, cy - 8),
                        cv2.FONT_HERSHEY_SIMPLEX, .6, (0, 255, 255), 1, cv2.LINE_AA)
        ...
        if k == ord(' '):   # finish
            break
        elif k in (ord('q'), 27):  # cancel
            selector.selL, selector.selR = [], []
            break
        elif k in (ord('r'), ord('R')):
            selector.selL, selector.selR = [], []

    cv2.destroyWindow(window)
    return selector.selL, selector.selR
